 ...
 * (that free-tier data is LinkedIn-only — no phone/email). A handful of
 * clearly-synthetic demo contacts are layered in on top so SMS/voice/email
 * actually have something to send to as well — every synthetic contact is
 * tagged source:"synthetic_test" end to end so it is never confused with
 * real prospect data.
 *
 * Exported so both `npm run seed` (scripts/seed.js, for local re-seeding)
 * and the server's own boot sequence (src/server.js, so a fresh Render
 * deploy — whose disk is wiped on every build — comes back up with the
 * required campaigns already running) call the exact same logic.
 */
const store = require("../state/store");
const { buildCampaign } = require("../campaigns/factory");
const { getIcpProspects } = require("../data/icpProspects");
const { runCampaign } = require("../agents/pipeline");
...
function prospectPoolFor(icpId) {
...
async function seedDemoData() {
  ...
  for (const id of ["camp_us_saas_cto", "camp_us_voice_ai_founders", "camp_india_bfsi_cio"]) {
    const campaign = store.getCampaign(id);
    campaign.status = "live";
    campaign.history.push({ ts: new Date().toISOString(), actor: "Arsh", action: "live" });
  ...
  // has real activity, so the dashboard shows a paused campaign that still
  // carries its own history rather than an empty shell.
  ...
  console.log("[seed] Reserved demo leads for live 'Add Prospect' testing:", RESERVED_DEMO_LEADS.map((l) => l.name).join(", "));
  console.log("[seed] Campaigns:", store.listCampaigns().map((c) => `${c.name} [${c.status}]`).join(" | "));
}

/** Seeds only if no campaigns exist yet — safe to call on every server boot. */
async function seedIfEmpty() {
  if (store.listCampaigns().length > 0) {
    console.log("[seed] existing campaign data found, skipping auto-seed");
    return;
  }
  console.log("[seed] no campaigns found — seeding demo data...");
  await seedDemoData();
}

module.exports = { seedDemoData, seedIfEmpty };
